...
unsigned tell (int fd)
{
  ...
  {
    unsigned i = (unsigned)file_tell (f);
    lock_release (&write_lock);
    return i;
  }
}
...
struct child* add_child_process (int pid)
{
  ...
  list_push_back(&thread_current()->offspring, &p->elem);
  return p;
}
...
struct child* get_child_process (int pid)
{
  ...
  struct list_elem *l;
  for (l = list_begin (&t->offspring); l != list_end (&t->offspring); l = list_next (l))
  {
    struct child *p = list_entry (l, struct child, elem);
    if (pid == p->pid)
      return p;
  }
  return NULL;
...
{
  struct thread *t = thread_current();
  struct list_elem *next, *l = list_begin (&t->offspring);

  while (l != list_end (&t->offspring))
    {
      next = list_next(l);
...
void check_valid_buffer (void* buffer, unsigned size)
{
  ...
  for (; i < size; i++)
    {
      check_valid_ptr((const void*) check);
      ++check;
    }
}
